...
CREATE SEQUENCE seq_orders_id START 1;
-- Create orders table
CREATE TABLE IF NOT EXISTS orders (
    order_id INT PRIMARY KEY DEFAULT nextval('seq_orders_id'),
...
CREATE SEQUENCE seq_products_id START 1;
CREATE TABLE IF NOT EXISTS products (
    product_id INT PRIMARY KEY DEFAULT nextval('seq_products_id'),
    name VARCHAR(100),
    description TEXT,
    price NUMERIC(10, 2),
    stock_quantity INT
...
CREATE TABLE IF NOT EXISTS order_items (
    order_item_id INT PRIMARY KEY DEFAULT nextval('seq_order_item_id'),
